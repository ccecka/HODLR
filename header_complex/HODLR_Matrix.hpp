...
public:
	...
	virtual std::complex<double> get_Matrix_Entry(const unsigned, const unsigned) {
         ...
         */
	void get_Matrix(const unsigned start_Row, const unsigned start_Col, const unsigned n_Rows, const unsigned n_Cols, MatrixXcd& A) {
		A   =   MatrixXcd(n_Rows,n_Cols);
         ...
         */
	void get_Matrix_Row(const unsigned start_Col, const unsigned n_Cols, const unsigned row_Index, VectorXcd& v) {
		v   =   VectorXcd(n_Cols);
		for (unsigned j=0; j<n_Cols; ++j) {
         ...
         */
	void get_Matrix_Col(const unsigned start_Row, const unsigned n_Rows, const unsigned col_Index, VectorXcd& v) {
		v   =   VectorXcd(n_Rows);
         ...
         */
	unsigned max_Abs_Vector(const VectorXcd& v, const vector<int>& not_Allowed_Indices, std::complex<double>& max) {
		unsigned n      =   v.size();
		max             =   v(0);
		unsigned index  =   0;
